...
static void EnumerateConstantRangesFP(unsigned ExpDiff, Fn TestFn) {
  ...
  for (APFloat Lo = NegInf; Lo.compare(PosInf) == APFloat::cmpLessThan;
               Lo = advanceFP(Lo, ExpDiff)) {
    for (APFloat Hi = NegInf; Hi.compare(PosInf) == APFloat::cmpLessThan;
                 Hi = advanceFP(Hi, ExpDiff)) {
      // Test with NaN
...
TEST_F(ConstantRangeTest, SingleElementFP) {
  ...
  EXPECT_TRUE(OneFP.getSingleElementFP()->bitwiseIsEqual(APFloat(10.0f)));
...
TEST(ConstantRange, MakeAllowedFCmpOEQRegion) {
  TestFPCompExhaustiveAllowed(
    ...
    [](const APFloat &LHS, const APFloat &RHS) {
      return !LHS.isNaN() && !RHS.isNaN() &&
             LHS.compare(RHS) == APFloat::cmpEqual; }
  );
}
...
TEST(ConstantRange, MakeAllowedFCmpOGTRegion) {
  TestFPCompExhaustiveAllowed(
    ...
    [](const APFloat &LHS, const APFloat &RHS) {
      return !LHS.isNaN() && !RHS.isNaN() &&
             LHS.compare(RHS) == APFloat::cmpGreaterThan; }
  );
}
...
TEST(ConstantRange, MakeAllowedFCmpOLTRegion) {
  TestFPCompExhaustiveAllowed(
    ...
    [](const APFloat &LHS, const APFloat &RHS) {
      return !LHS.isNaN() && !RHS.isNaN() &&
             LHS.compare(RHS) == APFloat::cmpLessThan; }
  );
}
