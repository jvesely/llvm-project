...
ConstantRange::ConstantRange(ConstantRange &&Other): isFloat(Other.isFloat), canBeNaN(Other.canBeNaN)
{
  if (isFloat) {
    new (&LowerFP) APFloat(std::move(Other.LowerFP));
    new (&UpperFP) APFloat(std::move(Other.UpperFP));
  } else {
    new (&Lower) APInt(std::move(Other.Lower));
    new (&Upper) APInt(std::move(Other.Upper));
  }
}

ConstantRange::ConstantRange(const ConstantRange &Other): isFloat(Other.isFloat), canBeNaN(Other.canBeNaN)
{
  if (isFloat) {
    new (&LowerFP) APFloat(Other.LowerFP);
    new (&UpperFP) APFloat(Other.UpperFP);
  } else {
    new (&Lower) APInt(Other.Lower);
    new (&Upper) APInt(Other.Upper);
  }
}

ConstantRange & ConstantRange::operator = (ConstantRange &&Other)
{
  if (isFloat) {
    LowerFP.~APFloat();
    UpperFP.~APFloat();
...
ConstantRange & ConstantRange::operator = (const ConstantRange &Other)
{
  if (isFloat) {
    LowerFP.~APFloat();
    UpperFP.~APFloat();
...
APFloat ConstantRange::getSignedMaxFP() const {
  assert(isFloat);
  if (isFullSet() || isUpperWrapped())
    return APFloat::getInf(LowerFP.getSemantics(), false);
  return getUpperFP();
}

APFloat ConstantRange::getSignedMinFP() const {
  assert(isFloat);
  if (isFullSet() || isUpperWrapped())
    return APFloat::getInf(LowerFP.getSemantics(), true);
  return getLowerFP();
}
...
bool ConstantRange::contains(const APFloat &V) const {
  ...
  auto CmpLo = LowerFP.compare(V);
  auto CmpHi = UpperFP.compare(V);
  if (!isUpperWrapped())
    return CmpLo == APFloat::cmpLessThan && CmpHi == APFloat::cmpGreaterThan;
  return CmpLo == APFloat::cmpLessThan || CmpHi == APFloat::cmpGreaterThan;
}
...
ConstantRange ConstantRange::intersectWith(const ConstantRange &CR,
  ...
  if (isUpperWrapped() && !CR.isUpperWrapped()) {
    ...
    if ((!isFloat && CR.Lower.ult(Upper)) ||
        (isFloat && (CR.LowerFP.compare(UpperFP) == APFloat::cmpLessThan ||
                     CR.LowerFP.compare(UpperFP) == APFloat::cmpEqual))) {
      // ------U   L--- : this
      //  L--U          : CR
      if (!isFloat && CR.Upper.ult(Upper))
        return CR;
      if (isFloat && CR.UpperFP.compare(UpperFP) == APFloat::cmpLessThan)
      ...
      if (!isFloat && CR.Upper.ule(Lower))
        return ConstantRange(CR.Lower, Upper);
      // FP range is inclusive so don't include == here
      if (isFloat && CR.UpperFP.compare(LowerFP) == APFloat::cmpLessThan)
    ...
    if ((!isFloat && CR.Lower.ult(Lower)) ||
        (isFloat && CR.LowerFP.compare(LowerFP) == APFloat::cmpLessThan)) {
      // --U      L---- : this
      //     L--U       : CR
      if (!isFloat && CR.Upper.ule(Lower))
        return getEmpty();
  ...
  if (isFloat) {
    assert(Type == Smallest);
    auto CmpUpLo = UpperFP.compare(CR.LowerFP);
    auto CmpLoUp = LowerFP.compare(CR.UpperFP);
    // Handle disjoint cases
    if (CmpUpLo == APFloat::cmpEqual || CmpUpLo == APFloat::cmpGreaterThan ||
        CmpLoUp == APFloat::cmpEqual || CmpLoUp == APFloat::cmpLessThan)
      return (CR.LowerFP - CR.UpperFP).compare(LowerFP - UpperFP) ==
              APFloat::cmpGreaterThan ? CR : *this;
    APFloat Upper = llvm::minimum(UpperFP, CR.UpperFP);
    APFloat Lower = llvm::maximum(LowerFP, CR.LowerFP);
...
ConstantRange ConstantRange::unionWith(const ConstantRange &CR,
  ...
  if (CR.isFloat && CR.UpperFP.isNaN()) return ConstantRange(LowerFP, UpperFP, true);
  if (isFloat && UpperFP.isNaN()) return ConstantRange(CR.LowerFP, CR.UpperFP, true);
  ...
  if (!CR.isUpperWrapped()) {
    ...
    if (isFloat) {
      auto CmpUpUp = UpperFP.compare(CR.UpperFP);
      if (CmpUpUp == APFloat::cmpGreaterThan || CmpUpUp == APFloat::cmpEqual)
        return ConstantRange(LowerFP, maximum(UpperFP, CR.UpperFP),
                             canBeNaN || CR.canBeNaN);
      auto CmpLoLo = LowerFP.compare(CR.LowerFP);
      if (CmpLoLo == APFloat::cmpLessThan || CmpLoLo == APFloat::cmpEqual)
        return ConstantRange(minimum(LowerFP, CR.LowerFP), UpperFP,
                             canBeNaN || CR.canBeNaN);
    }
    ...
    if (isFloat) {
      auto CmpUpLo = UpperFP.compare(CR.LowerFP);
      auto CmpLoUp = LowerFP.compare(CR.UpperFP);
      if ((CmpLoUp == APFloat::cmpLessThan || CmpLoUp == APFloat::cmpEqual) &&
          (CmpUpLo == APFloat::cmpGreaterThan || CmpUpLo == APFloat::cmpEqual))
    ...
    if (isFloat) {
      auto CmpUpLo = UpperFP.compare(CR.LowerFP);
      auto CmpLoUp = LowerFP.compare(CR.UpperFP);
      if (CmpUpLo == APFloat::cmpLessThan &&
          CmpLoUp == APFloat::cmpGreaterThan) {
        assert(Type == Smallest);
        return (CR.LowerFP - UpperFP).compare(LowerFP - CR.UpperFP) ==
